...
import type { RepairJob } from "@/types/AllTypes";
...
const RepairsTableSection: React.FC<RepairsTableSectionProps> = ({
  data = [],
  isLoading,
  currentPage,
  totalPages,
  ...
  // Transform API data to match RepairJob interface
  const transformedData: RepairJob[] = useMemo(() => {
    if (!data || !Array.isArray(data)) {
      return [];
    }
    return data.map((item) => ({
      id: item.repair_id,
      jobId: item.repair_id,
  ...
  const repairsColumns = [
    {
      header: "Job ID",
      accessor: "jobId" as keyof RepairJob,
    },
    {
      header: "Client",
      accessor: "client" as keyof RepairJob,
    },
    {
      header: "Model",
      accessor: "model" as keyof RepairJob,
    },
    {
      header: "Technician",
      accessor: "technician" as keyof RepairJob,
    },
    {
      header: "Scheduled",
      accessor: "scheduled" as keyof RepairJob,
    },
    {
      header: "Status",
      accessor: "status" as keyof RepairJob,
    },
    {
      header: "Action",
      accessor: (row: RepairJob) => (
